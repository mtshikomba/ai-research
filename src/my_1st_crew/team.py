...
class Crew:
    ...
    def __init__(self, project_root: str, dry_run: bool = False, safe_mode: bool = False, confirm_push: bool = False):
        ...
        # pass safety flags to git workflow
        ...
        # artifact store writes into target repo's artifacts/ directory
    ...
    def kickoff(self, inputs: Dict[str, Any]):
        ...
        for task_name, task_cfg in self.tasks.items():
            ...
            if task_name in self.ENGINEER_TASKS:
                # Respect dry-run: do not perform commit or PR actions
                if self.dry_run:
                    commit_result = {"status": "dry_run", "reason": "dry-run mode enabled; no commit performed"}
                    results[task_name]["git"] = commit_result
                    pr_result = {"status": "skipped", "reason": "dry-run; PR not created"}
                    results[task_name]["pull_request"] = pr_result
                else:
                    # Optionally confirm before committing
                    if self.confirm_push:
                        if not sys.stdin.isatty():
                            ...
                            if resp.strip().lower() != 'y':
                                ...
                                results[task_name]["git"] = commit_result
                                # Only create PRs when not in safe_mode and commit succeeded
            ...
            # Persist provenance inside the target repo's artifacts/ directory
            try:
                validation_record = {"valid": bool(valid), "errors": errors} if 'valid' in locals() else None
                ...
                # assemble a compact git metadata object
                ...
                # best-effort; do not fail the run on artifact saving errors
                pass
