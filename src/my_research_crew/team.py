...
class Crew:
    ...
    def __init__(
        ...
        candidate_config_1 = os.path.join(
            project_root, "src", "my_research_crew", "config"
        )
        ...
        if os.path.isdir(candidate_config_1):
            ...
            agent_repo_root = os.path.abspath(
        ...
        self.git_workflow = GitWorkflow(
            ...
            confirm_push=self.confirm_push,
        )
        ...
        self.validation_command = (
            self.project_context.test_command or "python -m compileall ."
        )
    ...
    def kickoff(self, inputs: Dict[str, Any]):
        ...
        for task_name, task_cfg in self.tasks.items():
            ...
            if task_name in self.ENGINEER_TASKS:
                branch_name = self.git_workflow.ensure_story_branch(
                    story, base_branch=self.project_context.default_branch
                )
                if branch_name:
            ...
            response = self.adapter.generate(
                prompt=prompt, expected=task_cfg.get("expected_output")
            )

            valid, errors = self.validator.validate(
                task_cfg.get("expected_output"), response
            )
            if not valid:
            ...
            if task_name in self.ENGINEER_TASKS:
                if self.dry_run:
                    commit_result = {
                        "status": "dry_run",
                        "reason": "dry-run mode enabled; no commit performed",
                    }
                    ...
                    if not sys.stdin.isatty():
                        print(
                            "Confirmation required to commit/push but no interactive TTY available; skipping commit/PR"
                        )
                        commit_result = {
                            "status": "skipped",
                            "reason": "confirmation required but no TTY",
                        }
                        results[task_name]["git"] = commit_result
                        results[task_name]["pull_request"] = {
                            "status": "skipped",
                        ...
                        resp = input(
                            f"Commit changes for task '{task_name}' on branch '{context.get('current_branch')}'? [y/N]: "
                        )
                        if resp.strip().lower() != "y":
                            ...
                            if commit_result.get("status") == "committed":
                                if self.safe_mode:
                                    pr_result = {
                                        "status": "skipped",
                                        "reason": "safe_mode enabled; automatic PR creation disabled",
                                    }
                                elif os.getenv("AUTO_CREATE_PR", "").lower() == "true":
                                    pr_result = self.git_workflow.create_pr_for_branch(
                                        story,
                                        base_branch=self.project_context.default_branch,
                                    )
                                else:
                                    pr_result = {
                                        "status": "skipped",
            ...
            try:
                ...
                git_meta = (
                    results[task_name].get("git")
                    if isinstance(results.get(task_name), dict)
                    else None
                )
                pr_meta = (
                    results[task_name].get("pull_request")
                    if isinstance(results.get(task_name), dict)
                    else None
                )
                git_info = (
                    {"commit": git_meta, "pr": pr_meta} if git_meta or pr_meta else None
                )
    ...
    def _build_prompt(
        ...
        backlog_summary = self.backlog_adapter.format_summary(
            context.get("backlog", {"stories": []})
        )
